...
class fps_camera:
    def __init__(self, position, target, up, fov, aspectRatio, near, far):
        ...
        self.translation_mat = np.array([0, 0, 0, 0])
        self.pitch = 0
        self.yaw = 0
        self.roll = 0
    ...
    def update_view(self):
        ...
        self.rot_mat = mat_rotate
        self.translation_mat = get_translate_mat(-self.pos)
        self.view_mat = np.matmul(mat_rotate, -self.translation_mat)
    ...
    def translate(self, translate_vec):
        # translate_vec = np.matmul(self.rot_mat, np.array(translate_vec))
        self.pos = self.pos + np.array(translate_vec)

    def rot_pos(self, sensitivity, first, last):
        xoff = (last[0] - first[0]) * sensitivity
        yoff = (first[1] - last[1]) * sensitivity
        self.setRotation_params(self.pitch + xoff, self.yaw + yoff)
        if self.pitch >= 89.9:
            self.pitch = 89
        ...
        direction = np.array([np.cos(yaw) * np.cos(pitch), np.sin(pitch), np.sin(yaw) * np.cos(pitch), 1])
